...
const MDProvince = () => {
  ...
  useEffect(() => {
    ...
    if (dataProvinces?.data?.province?.records) {
      // Filter data berdasarkan searchQuery
      const filteredData = dataProvinces?.data?.province?.records.filter((province) =>
        province.name.toLowerCase().includes(searchQuery.toLowerCase()),
      );
      setFilteredData(filteredData);
    }
  }, [dataProvinces, searchQuery]);
    ...
    <Box m="20px">
      ...
      >
        <div style={{ marginBottom: "5px" }}>
          <Box display="flex">
            <Typography variant="h3">Data Province</Typography>
      ...
      <EditProvinces isEditOpen={isEditOpen} onClose={() => setIsEditOpen(false)} dtProvince={selectedProvince} />
      <ViewProvinces isViewOpen={isViewOpen} onClose={() => setIsViewOpen(false)} dtProvince={selectedProvince} />
    </Box>
  );
};
