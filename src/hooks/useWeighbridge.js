...
export const useWeighbridge = () => {
  ...
  const initWB = () => {
    if (WBMS && WBMS?.WBMS_WB_IP && !wsClient) {
      wsClient = new w3cwebsocket(`ws://${WBMS?.WBMS_WB_IP}:${WBMS?.WBMS_WB_PORT}/GetWeight`);
      ...
      wsClient.onmessage = (message) => {
        ...
        } else if (moment().valueOf() - wb.lastChange > WBMS?.WBMS_WB_STABLE_PERIOD && !curWb.isStable) {
          isChange = true;
